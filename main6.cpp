...
class MIEM : private generator { //ticket SYYYYMMDDNNNNNC
    public:
...
class MGTUU : private generator { //ticket SYYYYMMDDNNNNC
    public:
        void set_sex(char s_given) override {
            if (s_given == 'M') {s = '2';
...
int main(int argc, char* argv[]) {
    ...
    if (mfromfile) {
        ...
        if (univercity == "MIEM") {
            MIEM new_ticket;
            char sex_char;
            if (sex == "man") {sex_char = 'M';} else if (sex == "woman") {sex_char = 'W';}
            new_ticket.set_sex(sex_char);
            new_ticket.set_birth_date(dd, mm, yyyy);
            new_ticket.set_magic_n();
            ticket = new_ticket.ticket();
        } else if (univercity == "MGTUU") {
            MGTUU new_ticket;
            char sex_char;
            if (sex == "man") {sex_char = 'M';} else if (sex == "woman") {sex_char = 'W';}
            new_ticket.set_sex(sex_char);
            ...
            std::cout << "Error!" << std::endl << "Input right univercity please." << std::endl;
            return -1;
        }
        if (mtofile) {
